...
The `check` task runs `lintKotlin` to check all files and also `installKotlinterPrePushHook` which adds a pre-push hook to check all changed kotlin files and if necessary format them.
See https://github.com/jeremymailen/kotlinter-gradle .

To run the linter manually use tasks `lintKotlin` and `formatKotlin`.
...
2. Add a link to the file you just created to [Release Notes](#release-notes)
   1. Note this should include `-beta`
...
2. Drop the `-beta` suffix from the version number
3. Update [Release Notes](#release-notes) to link to the non beta version
4. Commit and raise a PR
5. Post the PR link on #dps_dev for folks to review
6. After the new version has been published you can upgrade your remaining projects to the new non-beta version 
