...
class BaseConfigManager(override val project: Project) : ConfigManager {
  ...
  private fun getVersion(): String {
    return if (System.getenv().contains("BUILD_NUMBER")) { System.getenv("BUILD_NUMBER") } else { LocalDate.now().format(DateTimeFormatter.ISO_DATE) }
  }
  ...
  private fun setJunit5() {
    ...
    project.configurations.findByName("testImplementation")?.dependencies?.find { it.group == "junit" }?.version
      ...
      .run { project.dependencies.add("testImplementation", "org.junit.vintage:junit-vintage-engine:5.9.3") }
  }
}
